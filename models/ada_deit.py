...
from transformers.models.deit import *
...
class AdaDeiTEncoder(DeiTEncoder):
...
class AdaDeiTModel(DeiTModel):
    def __init__(
        ...
        del self.encoder
        self.encoder = AdaDeiTEncoder(config)
...
class AdaDeiTForImageClassification(torch.nn.Module):
    def __init__(
        self,
        image_processor_path="facebook/deit-base-distilled-patch16-224",
        ...
        if use_pretrained:
            self.model = AdaDeiTModel.from_pretrained(model_path)
        else:
            configuration_deit = DeiTConfig(num_classes=num_classes)
            self.model = AdaDeiTModel(configuration_deit)
...
class AdaDeiTForDensePrediction(torch.nn.Module):
    def __init__(
        self,
        image_processor_path="facebook/deit-base-distilled-patch16-224",
        model_path="facebook/deit-base-distilled-patch16-224",
        ...
        if self.use_pretrained:
            ...
            configuration_deit = DeiTConfig(num_classes=self.num_classes)
            self.model = AdaDeiTModel(configuration_deit)
